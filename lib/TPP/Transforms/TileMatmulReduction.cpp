...
namespace mlir {
namespace tpp {
#define GEN_PASS_DEF_TILEMATMULREDUCTION
...
struct TileReductionMatmulOp : public OpRewritePattern<linalg::MatmulOp> {
  using OpRewritePattern<linalg::MatmulOp>::OpRewritePattern;

  TileReductionMatmulOp(MLIRContext *ctx, TileMatmulReductionOptions options)
...
private:
  TileMatmulReductionOptions options;
};

// Transfer data from host to a GPU device.
struct TileMatmulReduction
    : public tpp::impl::TileMatmulReductionBase<TileMatmulReduction> {
  using TileMatmulReductionBase::TileMatmulReductionBase;

  void runOnOperation() override {
    MLIRContext *ctx = &getContext();

    TileMatmulReductionOptions options{tileSize};

    RewritePatternSet patterns(ctx);
    patterns.add<TileReductionMatmulOp>(ctx, options);
