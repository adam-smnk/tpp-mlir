...
module attributes {gpu.container_module} {
  func.func @entry(%arg0: memref<4096x4096xf32>, %arg1: memref<4096x4096xf32>, %arg2: memref<4096x4096xf32>) {
    ...
    gpu.launch_func  @entry_kernel::@entry_kernel blocks in (%c32, %c32, %c1) threads in (%c16, %c16, %c1)  args(%arg0 : memref<4096x4096xf32>, %arg1 : memref<4096x4096xf32>, %arg2 : memref<4096x4096xf32>, %c0 : index, %c128 : index, %c1 : index)
    return
  }
  gpu.module @entry_kernel {
    // PARAM: SMEM caches have to match <(GEMM tile size)x(reduction dim step)>
    memref.global "private" @smemTileA : memref<128x8xf32, #gpu.address_space<workgroup>>
    memref.global "private" @smemTileB : memref<8x128xf32, #gpu.address_space<workgroup>>
    ...
    gpu.func @entry_kernel(%arg0: memref<4096x4096xf32>, %arg1: memref<4096x4096xf32>, %arg2: memref<4096x4096xf32>, %arg3: index, %arg4: index, %arg5: index) kernel attributes {gpu.known_block_size = array<i32: 16, 16, 1>, gpu.known_grid_size = array<i32: 2, 2, 1>} {
      ...
      %swizzleSize = arith.constant 4 : index
      ...
      %tileRowC = arith.addi %rowOffsetC, %swizzleRow : index
      ...
      %tileColC = arith.addi %colOffsetC, %swizzleCol : index
      ...
      %smemA = memref.get_global @smemTileA : memref<128x8xf32, #gpu.address_space<workgroup>>
      %smemB = memref.get_global @smemTileB : memref<8x128xf32, #gpu.address_space<workgroup>>

      // Thread tile sizes.
      // Each thread will compute <4x4> elements of C tile.
      %TM = arith.constant 8 : index
      %TN = arith.constant 8 : index
      ...
      scf.for %tm = %c0 to %TM step %c1 {
        scf.for %tn = %c0 to %TN step %c1 {
          ...
          %elemC = memref.load %subview_1[%cRow, %cCol] : memref<128x128xf32, strided<[4096, 1], offset: ?>>
      ...
      scf.for %subtileIv = %c0 to %numSubTilesK step %c1 {
        ...
        scf.for %subtileStep = %c0 to %numStepsB step %c1 {
          ...
          %elemB = memref.load %subview_0[%rowB, %tColB] : memref<4096x128xf32, strided<[4096, 1], offset: ?>>
          memref.store %elemB, %smemB[%smemRowB, %tColB] : memref<8x128xf32, #gpu.address_space<workgroup>>
        }
        ...
        scf.for %offset = %c0 to %BK step %c1 {
          scf.for %tm = %c0 to %TM step %c1 {
            ...
            %elemA = memref.load %smemA[%smemRowA, %offset] : memref<128x8xf32, #gpu.address_space<workgroup>>
          ...
          scf.for %tn = %c0 to %TN step %c1 {
            ...
            %elemB = memref.load %smemB[%offset, %smemColB] : memref<8x128xf32, #gpu.address_space<workgroup>>
      ...
      scf.for %tm = %c0 to %TM step %c1 {
        scf.for %tn = %c0 to %TN step %c1 {
          ...
          memref.store %res, %subview_1[%cRow, %cCol] : memref<128x128xf32, strided<[4096, 1], offset: ?>>
        }
      }
